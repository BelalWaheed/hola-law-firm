...
import { checkConsultationStatus } from "../api/landing.api";
...
export const Navbar: React.FC<NavbarProps> = ({ settings }) => {
    ...
    try {
      ...
      if (response.success && Array.isArray(response.data)) {
        ...
        const unreadItems = response.data.filter(
          (item: { _id: string; adminReply?: string }) => item.adminReply && !seenIds.includes(item._id)
        );
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("hola_seen_replies_updated", checkNotifications);
    ...
    <>
      ...
      <nav className={`navbar ${scrolled ? "scrolled" : ""}`} id="navbar">
        <div className="w-full max-w-7xl mx-auto  flex items-center justify-between">
          ...
          >
            <Logo size={40} />
            <span className="whitespace-nowrap">{settings?.navbarBrand || "عبدالمنعم أبو السعود السيد"}</span>
          </Link>
          ...
          <div className="flex items-center gap-4">
            {unreadCount > 0 && (
              <Link
                to={firstUnreadId ? `/status?replyId=${firstUnreadId}` : "/status"}
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-border-color pb-4 mb-6">
          <span className="font-amiri font-bold text-accent-gold text-lg">القائمة</span>
          <button
            onClick={() => setMobileMenuOpen(false)}
